...
class MultisetSpec extends FlatSpec with MustMatchers {
  ...
  "Multiset" should "be covariant" in {
    class School(val name: String) {
      override def equals(other: scala.Any): Boolean =
        other match {
          case other: School => this.name == other.name
          case _ => false
        }
    }

    case class Lyceum(override val name: String, specialization: String) extends School(name)

    val lyceums: Multiset[Lyceum] =
      Multiset(Lyceum("15", "Math/physics"), Lyceum("15", "fake"))
    val schools: Multiset[School] =
      lyceums

    schools must === (Multiset(new School("15"), new School("15")))
  }
}
